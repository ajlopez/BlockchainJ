...
public class Block {
    ...
    public Block(long number, BlockHash parentHash, List<BlockHeader> uncles, List<Transaction> transactions, Hash receiptsRootHash, Hash stateRootHash, long timestamp, Address coinbase, Difficulty difficulty, long gasLimit) {
        this(new BlockHeader(number, parentHash, transactions == null ? 0 : transactions.size(), calculateTransactionsRootHash(transactions), receiptsRootHash, uncles == null ? 0 : uncles.size(), calculateUnclesRootHash(uncles), stateRootHash, timestamp, coinbase, difficulty, gasLimit, 0), uncles, transactions);
    }
